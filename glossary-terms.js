const GLOSSARY_TERMS = [
  ...
  {t:"Generative Orchestration", d:"The default Copilot Studio mode where the LLM dynamically selects topics, knowledge sources, and tools per turn based on their descriptions, rather than following fixed trigger-phrase routing. Best for open intents and multi-step reasoning.", tags:["Copilot Studio","AI Agents"]},
  {t:"Slot Filling", d:"A Copilot Studio behavior where the agent keeps prompting until all required input variables for a topic or action are collected. The generative orchestrator can fill slots from natural language instead of one question per field.", tags:["Copilot Studio","AI Agents"]},
  {t:"Prompt Action", d:"A reusable LLM prompt (summarize, classify, extract, rewrite) exposed to a Copilot Studio agent as a first-class, typed tool. Built and versioned in AI Builder's prompt editor.", tags:["Copilot Studio","AI Agents"]},
  {t:"Test Pane", d:"The side panel in the Copilot Studio maker portal that re-runs the agent on every save and exposes a per-turn trace &mdash; topic chosen, tools called, arguments passed, and knowledge passages retrieved. The primary debugging surface.", tags:["Copilot Studio"]},
  {t:"Tool Description", d:"The natural-language text attached to a Copilot Studio action or knowledge source that the generative orchestrator reads to decide when to use it. Weak descriptions are the most common cause of an agent ignoring or misusing a tool.", tags:["Copilot Studio","AI Agents"]},
  {t:"Solution (Power Platform)", d:"A packaged zip of Power Platform / Copilot Studio artifacts (agents, flows, connection references) used for source control and application lifecycle management. Promoted between Dev, Test, and Prod environments via pipelines.", tags:["Power Platform","Microsoft 365","ALM"]},
  {t:"Power Platform Environment", d:"An isolated container for apps, agents, and data with its own Dataverse, DLP policy, and capacity. The boundary for governance in Copilot Studio &mdash; makers build in Dev and promote to Prod.", tags:["Power Platform","Microsoft 365","Governance"]},
];
