const GLOSSARY_TERMS = [
  ...
  {t:"Agent SDK", d:"Anthropic library that embeds Claude Code's agent loop in your process. Official packages: <code>@anthropic-ai/claude-agent-sdk</code> (JS/TS) and <code>claude-agent-sdk</code> (Python). Other languages drive the same CLI with <code>-p</code> and JSON output.", tags:["Agent SDK","Claude Code"]},
  {t:"Agent Loop", d:"The cycle the Agent SDK runs: receive prompt, Claude may emit tool calls, the SDK executes tools (with permissions and hooks), results feed back, repeat until a text-only response, then a ResultMessage. Each tool-use round trip is one turn.", tags:["Agent SDK"]},
  {t:"query()", d:"Primary Agent SDK entry point. Takes a prompt (string or async iterable) and Options, returns an async generator of SDK messages plus control methods such as <code>interrupt()</code> and <code>setModel()</code>.", tags:["Agent SDK","JavaScript"]},
